...
public class JXWindow extends JXComponent{
     ...
     */
    public JXWindow(String name, int width, int height){
    ...
    public JXWindow(String name, int width, int height, JXLoadingScreen screen){
        logger.debug("Creating Window: '" + name + "'");
        window = new JFrame(name);
        window.setSize(width, height);
        window.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
     ...
     * @param JXMenu: JMenuBar
     */
    public void addBar(JXMenu JXMenu){
        logger.debug("Adding Menu Bar to Window: '" + window.getName() + "'");
        window.setJMenuBar(JXMenu.getBar());
    }
